...
@section('content')
    ...
    <section class="mt-8 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        @if (auth()->user()->role === \App\Models\User::ROLE_ADMIN)
            <form method="POST" action="{{ route('ponds.devices.store', $pond) }}" data-cy="register-device-form" class="mt-6 grid gap-4 rounded-xl bg-slate-50 p-5 md:grid-cols-[1fr_1fr_auto] md:items-end">
                @csrf
                <div>
    ...
    @if (auth()->user()->role === \App\Models\User::ROLE_ADMIN)
        <section class="mt-8 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <form method="POST" action="{{ route('ponds.thresholds.store', $pond) }}" data-cy="threshold-form" class="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                @csrf
                @foreach ([
    ...
    <section class="mt-8 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div class="mt-5 space-y-4">
            @forelse ($activeAlerts as $alert)
                <article data-cy="active-alert" class="rounded-xl border border-amber-200 bg-amber-50 p-4">
                    <div class="flex flex-wrap items-start justify-between gap-3">
                        <div>
                            ...
                            @if ($alert->assignedTo)
                                <p data-cy="assigned-specialist" class="mt-2 text-sm font-medium text-amber-900">
                                    Especialista asignado: {{ $alert->assignedTo->name }}
                                </p>
                            @endif
                        </div>
                        <span data-cy="alert-status" class="rounded-full bg-amber-200 px-3 py-1 text-xs font-semibold uppercase text-amber-900">
                            {{ $alert->status }}
                        </span>
                    </div>
                    ...
                    @if (
                        ...
                        <form method="POST" action="{{ route('alerts.assign', $alert) }}" class="mt-4 flex flex-wrap items-end gap-3">
                            ...
                            <div class="min-w-64 flex-1">
                                ...
                                <select id="specialist_{{ $alert->id }}" name="specialist_id" data-cy="specialist-select" required
                            ...
                            <button type="submit" data-cy="assign-incident" class="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-700">
                                Asignar incidencia
                            </button>
                        </form>
                    @endif
                    ...
                    @if (
                        ...
                        <form method="POST" action="{{ route('alerts.resolve', $alert) }}" data-cy="resolution-form" class="mt-4">
                            @csrf
                            ...
                            @error('resolution_notes')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                            <button type="submit" data-cy="resolve-incident" class="mt-3 rounded-lg bg-amber-700 px-4 py-2 text-sm font-semibold text-white hover:bg-amber-600">
                                Resolver incidencia
                            </button>
                        </form>
                    @endif
            ...
            @empty
                <p data-cy="no-active-alerts" class="text-sm text-slate-500">No hay alertas activas.</p>
            @endforelse
        </div>
    </section>

    <section data-cy="incident-history" class="mt-8 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="border-b border-slate-200 px-6 py-4">
